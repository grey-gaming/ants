...
function determineStatus(err: unknown): ContentfulStatusCode {
	const name = err instanceof Error ? err.name : "";
	if (name in ERROR_MAP) return ERROR_MAP[name] as ContentfulStatusCode;
	// Check for ZodError (any form)
	if (name === "ZodError") return 422 as ContentfulStatusCode;
	// Check error message for ZodError
	if (err instanceof Error && err.message.includes("ZodError"))
		return 422 as ContentfulStatusCode;
...
export function registerErrorHandler(app: unknown) {
	const h = app as {
		onError: (fn: (err: unknown, c: Context<AppEnv>) => Response) => void;
	};
	h.onError((err, c) => {
		const status = determineStatus(err);
		const name = err instanceof Error ? err.name : "Unknown";
