...
import { type LLMProvider, MlxProvider } from "@ants/llm";
import { $db } from "@ants/store";
import { toolRegistry } from "@ants/tools";
import type { PostgresJsDatabase } from "drizzle-orm/postgres-js";
import { Hono } from "hono";
import type { Env } from "hono/types";
import { type ConfiguredServices, configureServices } from "./app";
...
const sharedWorker: ReturnType<typeof createQueueWorker> | null = null;
...
export function buildApp(dbOnly?: PostgresJsDatabase): Hono<AppEnv> & {
	...
	void discoverAndRegister(db, toolEntries, agentEntries)
		.then((result) => {
			discoveryDone = true;
			const toolsMsg =
				result.toolsRegistered.length > 0
	...
	const llmProvider = new MlxProvider({
		...
		modelName:
			process.env.MLX_MODEL_NAME || "mlx-community/Llama-3.2-3B-Instruct-4bit",
	...
	const app = new Hono<AppEnv>() as Hono<AppEnv> & {
	...
	app.route(
		...
		createRunStepsRoutes(db, services),
	);
