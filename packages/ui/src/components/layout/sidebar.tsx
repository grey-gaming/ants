...
import { useCreateThread, useThreads } from "@/hooks/api";
	...
	return (
		...
		>
			<div className="flex h-16 items-center justify-between px-4">
				...
				{isCollapsed && (
					<div className="mx-auto flex h-8 w-8 items-center justify-center rounded-md bg-accent text-white">
						A
					</div>
				)}
				<Button
						...
						>
							<CollapsibleTrigger asChild>
								<Button
									variant="ghost"
									className={cn(
										"mb-1 w-full justify-start gap-2 px-2 text-text-secondary hover:text-text-primary",
										isCollapsed && "px-1",
									)}
								>
						...
						>
							<CollapsibleTrigger asChild>
								<Button
									variant="ghost"
									className={cn(
										"mb-1 mt-2 w-full justify-start gap-2 px-2 text-text-secondary hover:text-text-primary",
										isCollapsed && "px-1",
									)}
								>
	...
	return (
		<Link to={to}>
			<Button
				variant="ghost"
				className={cn(
					"w-full justify-start gap-2 px-2 text-text-secondary hover:text-text-primary",
					collapsed && "px-1",
				)}
			>
...
function SidebarThread({
	...
	return (
		<Link to="/threads/$threadId" params={{ threadId: thread.id }}>
			<div
				className={cn(
					"group mb-1 rounded-md px-3 py-2 transition-colors hover:bg-surface-2",
					thread.activeRunId && "bg-surface-2",
				)}
			>
				...
				{!collapsed && thread.lastMessageSnippet && (
