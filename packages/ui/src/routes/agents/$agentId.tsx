...
import { ArrowLeft, Wrench, Edit, Trash2, Loader2 } from 'lucide-react'
import { useAgent, mapAgentStatus } from '@/hooks/api'
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <AgentAvatar name={agent.name} tier={agent.tier} status={mapAgentStatus(agent.status)} size="lg" />
            <div>
              <h1 className="text-heading-lg text-text-primary">{agent.name}</h1>
              <div className="flex items-center gap-2 mt-1">
                <Badge variant="secondary">{agent.tier.toUpperCase()} Specialist</Badge>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              {agent.tools.map((tool) => (
                <div key={tool} className="flex items-center justify-between rounded-md border border-border p-3">
                  <div className="flex items-center gap-3">
                    <Wrench className="h-4 w-4 text-text-tertiary" />
                    <span className="text-sm text-text-primary capitalize">{tool.replace('_', ' ')}</span>
                  </div>
        ...
        <TabsContent value="runs">
          <Card>
            ...
            <CardContent>
              <p className="text-sm text-text-tertiary">Run history will appear here when runs are completed.</p>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
