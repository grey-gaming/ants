...
import { useThreads, useAgents, useCreateThread } from '@/hooks/api'
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatCard label="Active Runs" value={activeRuns} icon={Activity} color="text-info" />
            <StatCard label="Threads" value={threadCount} icon={MessageSquare} color="text-accent" />
            <StatCard label="Agents Online" value={`${agentsOnline}/${agentsTotal}`} icon={Bot} color="text-success" />
            <StatCard label="Errors" value={errors} icon={AlertCircle} color="text-error" />
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle className="text-heading-md">Recent Activity</CardTitle>
              </CardHeader>
                ...
                {recentThreads.map((thread) => (
                  ...
                  >
                    <div className={cn(
                      ...
                      thread.activeRunId ? 'bg-warning animate-pulse' : 'bg-text-tertiary'
                    )} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-text-primary truncate">{thread.title}</p>
                      <p className="text-xs text-text-tertiary">{formatTime(thread.updatedAt)}</p>
                    </div>
                    <Badge variant={thread.activeRunId ? 'default' : 'secondary'}>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                {!agents?.length && (
                  <p className="text-sm text-text-tertiary">No agents registered.</p>
                )}
              </CardContent>
            </Card>
          </div>
