...
export const queryKeys = {
	...
	runs: (cursor?: string) => ["runs", cursor] as const,
...
export function useRuns(cursor?: string) {
	return useQuery<{ data: Run[]; nextCursor: string | null }, Error>({
		queryKey: queryKeys.runs(cursor),
		queryFn: () => getRuns(cursor),
	});
}
	...
	return useMutation<Thread, Error, void>({
		...
		onSuccess: (data) => {
			queryClient.setQueryData(
				queryKeys.threads,
				(old: Thread[] | undefined) => (old ? [data, ...old] : [data]),
			);
		...
		onSuccess: (updated, variables) => {
			...
			queryClient.setQueryData(
				...
				(old: Thread[] | undefined) =>
					old
						? old.map((t) => (t.id === variables.id ? updated : t))
						: undefined,
			);
		},
	});
...
export function useSendMessage() {
	...
	return useMutation<Message, Error, { threadId: string; content: string }>({
		...
		onSuccess: (data, variables) => {
			queryClient.setQueryData(
				queryKeys.messages(variables.threadId),
...
export function useUpdateAgent() {
	...
	return useMutation<
		AgentType,
		Error,
		{ id: string; data: Partial<AgentType> }
	>({
		...
		onSuccess: (updated, variables) => {
			...
			queryClient.setQueryData(
				queryKeys.agents,
				(old: AgentType[] | undefined) =>
					old
						? old.map((a) => (a.id === variables.id ? updated : a))
						: undefined,
			);
		},
	});
...
export function useUpdateSetting() {
	...
	>({
		...
		onSuccess: (updated, variables) => {
			...
			queryClient.setQueryData(
				queryKeys.settings,
				(old: Setting[] | undefined) =>
					old
						? old.map((s) => (s.key === variables.key ? updated : s))
						: undefined,
			);
		},
	});
...
export function useRegisterTool() {
	...
	>({
		mutationFn: ({ name, description, type, config }) =>
			registerTool({ name, description, type, config }),
		onSuccess: () => {
...
export function useDefaultModel() {
	return useQuery<string | null, Error>({
		...
		queryFn: () =>
			getSetting("default_model").then((s) => s?.value?.model as string | null),
		retry: false,
	});
}
...
export function useSetDefaultModel() {
	...
	return useMutation<Setting, Error, { model: string }>({
		mutationFn: ({ model }) =>
			updateSetting("default_model", { value: { model } }),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["settings", "default_model"],
			});
...
export function useCreateRun() {
	...
	return useMutation<Run, Error, { threadId: string; agentTypeId: string }>({
		...
		onSuccess: (data) => {
			queryClient.invalidateQueries({
				queryKey: queryKeys.activity(data.threadId),
			});
		},
	});
}
