import { estimateTokens, logger, ValidationError } from "@ants/core";
...
export abstract class BaseAgent implements Agent {
	...
	async run(messages: Message[], tools: Tool[]): Promise<AgentResult> {
		logger.info(
			"agents",
			`Running agent "${this.name}" with ${messages.length} message(s)`,
		);
		const result = await this._execute(messages, tools);
		const tokens = estimateTokens(result.content);
		logger.info(
			"agents",
			`${this.name} produced ${result.content.length} char(s) of output`,
		);
		return {
			...result,
