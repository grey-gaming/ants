import { logger, NotFoundError, ValidationError } from "@ants/core";
...
export class OrchestratorAgent extends BaseAgent {
	...
	protected async _execute(
		...
		const tier =
			messages.length > 0 &&
			messages[messages.length - 1].content.includes("T2")
				? "T2"
				: "T3";
		logger.info(
			"agents",
			`${this.name} delegating to ${this.delegate.name} for tier ${tier}`,
		);
