...
import { tools, agentTypes } from "@ants/store";
...
export async function discoverAndRegister(
  ...
  for (const entry of toolEntries) {
    ...
    if (existing.length > 0) {
      logger.info("discovery", `Tool "${entry.name}" already registered — skipping`);
    ...
    try {
      ...
      logger.warn("discovery", `Failed to register tool "${entry.name}": ${err instanceof Error ? err.message : String(err)}`);
      result.toolsSkipped.push(entry.name);
    }
  }
  ...
  for (const entry of agentEntries) {
    ...
    if (existing.length > 0) {
      logger.info("discovery", `Agent "${entry.name}" already registered — skipping`);
    ...
    try {
      ...
      logger.info("discovery", `Registered agent: ${entry.name} (${entry.tier})`);
      result.agentsRegistered.push(entry.name);
    } catch (err) {
      logger.warn("discovery", `Failed to register agent "${entry.name}": ${err instanceof Error ? err.message : String(err)}`);
      result.agentsSkipped.push(entry.name);
    }
  }
