...
function makeMockDb(
	...
	function resolvedEmpty(): thenable {
		return Object.assign(async () => emptyResult, {
...
describe("queue-service", () => {
	...
	test("enforceConcurrencyLimits throws on global limit", async () => {
		...
		await expect(service.enforceConcurrencyLimits()).rejects.toThrow(
			RateLimitError,
		);
	});
	...
	test("enforceConcurrencyLimits throws on per-user limit", async () => {
		...
		await expect(service.enforceConcurrencyLimits("u-1")).rejects.toThrow(
			RateLimitError,
		);
	});
	...
	test("enforceConcurrencyLimits allows within limits", async () => {
		...
		await expect(
			service.enforceConcurrencyLimits("u-1"),
		).resolves.toBeUndefined();
	});
