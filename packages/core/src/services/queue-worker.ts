...
export function createQueueWorker(
	...
	async function dequeue(): Promise<{
		...
		const [job] = await db
			...
			.orderBy(
				sql`CASE job_queue.priority WHEN 'critical' THEN 0 WHEN 'high' THEN 1 WHEN 'normal' THEN 2 WHEN 'low' THEN 3 END`,
			)
			.limit(1);
	...
	async function processOne(): Promise<void> {
		...
		logger.info(
			"queue-worker",
			`Dispatching run ${result.runId} (priority: ${result.priority})`,
		);
		...
		while (retries < maxRetries && !success) {
			...
			try {
				...
				logger.warn(
					"queue-worker",
					`Retry ${retries}/${maxRetries} for run ${result.runId}: ${msg}`,
				);
			}
		}
