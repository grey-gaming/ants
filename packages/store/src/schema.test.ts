  ...
  test('runs table has correct columns', () => {
    ...
    expect(names).toEqual(
      expect.arrayContaining([
        'id', 'thread_id', 'user_id', 'agent_type_id', 'parent_run_id', 'status',
...
describe('Schema — Enum Definitions', () => {
  ...
  test('runStepStatusEnum has correct values', () => {
    expect(runStepStatusEnum.enumValues).toEqual(['in_progress', 'completed', 'failed']);
  });

  test('toolCallStatusEnum has correct values', () => {
    expect(toolCallStatusEnum.enumValues).toEqual(['in_progress', 'completed', 'failed']);
  });
  ...
  test('jobQueuePriorityEnum has correct values', () => {
    expect(jobQueuePriorityEnum.enumValues).toEqual(['critical', 'high', 'normal', 'low']);
  });
});
