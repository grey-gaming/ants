...
export const threads = pgTable('threads', {
  ...
  metadata: jsonb('metadata', { mode: 'json' }).$type<Record<string, unknown>>(),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
}, (t) => ({
...
export const agentTypes = pgTable('agent_types', {
  ...
  modelConfig: jsonb('model_config', { mode: 'json' }).$type<Record<string, unknown>>(),
  capabilities: jsonb('capabilities', { mode: 'json' }).$type<Record<string, unknown>>(),
...
export const settings = pgTable('settings', {
  ...
  value: jsonb('value', { mode: 'json' }).$type<Record<string, unknown>>(),
...
export const runs = pgTable('runs', {
  ...
  modelConfig: jsonb('model_config', { mode: 'json' }).$type<Record<string, unknown>>(),
  usage: jsonb('usage', { mode: 'json' }).$type<Record<string, unknown>>(),
...
export const runSteps = pgTable('run_steps', {
  ...
  details: jsonb('details', { mode: 'json' }).$type<Record<string, unknown>>(),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  completedAt: timestamp('completed_at'),
}, (t) => ({
...
export const toolCalls = pgTable('tool_calls', {
  ...
  name: text('name').notNull(),
  arguments: jsonb('arguments', { mode: 'json' }).$type<Record<string, unknown>>(),
  result: jsonb('result', { mode: 'json' }).$type<Record<string, unknown>>(),
