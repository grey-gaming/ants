...
export const threads = pgTable(
	"threads",
	{
		...
		runCount: varchar("run_count", { length: 10 }).$type<number>(),
...
export const agentTypes = pgTable(
	"agent_types",
	{
		...
		model: text("model"),
		delegatesTo: text("delegates_to").array().default([]),
...
export const tools = pgTable(
	"tools",
	{
		...
		description: text("description").notNull(),
		parametersSchema:
			jsonb("parameters_schema").$type<Record<string, unknown>>(),
...
export const toolCallsRelations = relations(toolCalls, ({ one }) => ({
	runStep: one(runSteps, {
		fields: [toolCalls.runStepId],
		references: [runSteps.id],
	}),
