...
async function step2_createThread() {
	...
	if (!res.ok) {
		throw new Error(
			`Failed to create thread: ${res.status} ${await res.text()}`,
		);
	}
...
async function step3_listThreads(threadId: string) {
	...
	if (!res.ok) {
		throw new Error(
			`Failed to list threads: ${res.status} ${await res.text()}`,
		);
	}
	...
	console.log(
		`  ✓ Found ${threads.length} thread(s), our thread: ${found ? "✓ present" : "✗ missing"}`,
	);
	return threads;
}
...
async function step4_getThread(threadId: string) {
	...
	const data = await res.json();
	console.log(
		`  ✓ Thread retrieved: title="${data.title}", created=${data.createdAt}`,
	);
	return data;
}
...
async function step5_updateThread(threadId: string) {
	...
	if (!res.ok) {
		throw new Error(
			`Failed to update thread: ${res.status} ${await res.text()}`,
		);
	}

	const data = await res.json();
	console.log(
		`  ✓ Thread updated: title="${data.title}", isPinned=${data.isPinned}`,
	);
	return data;
}
...
async function step6_sendMessage(threadId: string) {
	...
	const res = await fetch(`${BASE_URL}/messages`, {
		...
		body: JSON.stringify({
			...
			role: "user",
			content:
				"Hello, ANTS! This is an end-to-end test message to verify the UI hooks work correctly.",
		}),
	});
...
async function step7_listMessages(threadId: string) {
	...
	if (!res.ok) {
		throw new Error(
			`Failed to list messages: ${res.status} ${await res.text()}`,
		);
	}
...
async function step8_listAgents() {
	...
	agents.forEach((a: any) =>
		console.log(`    • ${a.name} (${a.tier}) - Status: ${a.status}`),
	);
	return agents;
}
...
async function step9_getAgent(agentId: string) {
	...
	const data = await res.json();
	console.log(
		`  ✓ Agent details: ${data.name} - Model: ${data.modelConfig?.model || "N/A"}`,
	);
	return data;
}
...
async function step11_getThreadActivity(threadId: string) {
	...
	if (!res.ok) {
		throw new Error(
			`Failed to get activity: ${res.status} ${await res.text()}`,
		);
	}

	const data = await res.json();
	console.log(
		`  ✓ Activity retrieved: ${data.totalRuns} runs, ${data.runs.length} root run(s)`,
	);
	return data;
}
...
async function step12_deleteThread(threadId: string) {
	...
	try {
		...
		if (!res.ok) {
			...
			console.log(
				`  ⚠ Thread not deleted (messages exist, FK constraint): ${res.status}`,
			);
		} else {
