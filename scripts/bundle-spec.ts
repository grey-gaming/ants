...
function readAndMergeDir(dirPath: string): Record<string, unknown> {
  ...
  const files = readdirSync(dirPath).filter((f) => f.endsWith(".yaml")).sort();
...
function main(): void {
  const spec = readYamlFile(join(OPENAPI_DIR, "spec.yaml")) as Record<string, unknown>;

  const paths = readAndMergeDir(join(OPENAPI_DIR, "paths"));
  const schemas = readAndMergeDir(join(OPENAPI_DIR, "schemas"));
  const parameters = readYamlFile(join(OPENAPI_DIR, "parameters.yaml")) as Record<string, unknown>;
  const responses = readYamlFile(join(OPENAPI_DIR, "responses.yaml")) as Record<string, unknown>;

  const resolvedPaths = resolveRefs(paths, join(OPENAPI_DIR, "paths")) as Record<string, unknown>;
  const resolvedSchemas = resolveRefs(schemas, join(OPENAPI_DIR, "schemas")) as Record<string, unknown>;
  const resolvedParameters = resolveRefs(parameters, OPENAPI_DIR) as Record<string, unknown>;
  const resolvedResponses = resolveRefs(responses, OPENAPI_DIR) as Record<string, unknown>;
